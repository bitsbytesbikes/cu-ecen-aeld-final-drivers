...
static struct i2c_client *aesd_bme_i2c_client_bmp = NULL;
...
static dev_t aesd_bmp280_dev = 0;
	...
	if ((rslt == BMP280_OK) && (reg_addr != NULL) && (reg_data != NULL)) {
		if (len != 0) {
			...
			if (dev->intf == BMP280_SPI_INTF) {
                 ...
                 */
				for (reg_addr_cnt = 0; reg_addr_cnt < len;
				     reg_addr_cnt++) {
					reg_addr[reg_addr_cnt] =
						reg_addr[reg_addr_cnt] & 0x7F;
				}
			}
			...
			if (len > 1) {
				...
				interleave_data(reg_addr, temp_buff, reg_data,
						len);
				temp_len = ((len * 2) - 1);
			} else {
				temp_len = len;
			}
			rslt = dev->write(dev->dev_id, reg_addr[0], temp_buff,
					  temp_len);
...
int8_t bmp280_init(struct bmp280_dev *dev)
{
	...
	if (rslt == BMP280_OK) {
		while (try_count) {
			rslt = bmp280_get_regs(BMP280_CHIP_ID_ADDR,
					       &dev->chip_id, 1, dev);
			pr_info("rslt : %d chip id: %d \n", rslt, dev->chip_id);

			/* Check for chip id validity */
			if ((rslt == BMP280_OK) &&
			    (dev->chip_id == BMP280_CHIP_ID1 ||
...
int8_t bmp280_get_config(struct bmp280_config *conf, struct bmp280_dev *dev)
{
	...
	if ((rslt == BMP280_OK) && (conf != NULL)) {
		...
		if (rslt == BMP280_OK) {
			conf->os_temp =
				BMP280_GET_BITS(BMP280_OS_TEMP, temp[0]);
			conf->os_pres =
				BMP280_GET_BITS(BMP280_OS_PRES, temp[0]);
			conf->odr =
				BMP280_GET_BITS(BMP280_STANDBY_DURN, temp[1]);
			conf->filter = BMP280_GET_BITS(BMP280_FILTER, temp[1]);
			conf->spi3w_en = BMP280_GET_BITS_POS_0(
				BMP280_SPI3_ENABLE, temp[1]);
			dev->conf = *conf;
		}
	} else {
		rslt = BMP280_E_NULL_PTR;
	...
	if ((rslt == BMP280_OK) && (uncomp_data != NULL)) {
		...
		if (rslt == BMP280_OK) {
			uncomp_data->uncomp_press =
				(int32_t)((((uint32_t)(temp[0])) << 12) |
					  (((uint32_t)(temp[1])) << 4) |
					  ((uint32_t)temp[2] >> 4));
			uncomp_data->uncomp_temp =
				(int32_t)((((int32_t)(temp[3])) << 12) |
					  (((int32_t)(temp[4])) << 4) |
					  (((int32_t)(temp[5])) >> 4));
			rslt = st_check_boundaries(
				  ...
				  struct bmp280_dev *dev)
{
	...
	if (rslt == BMP280_OK) {
		var1 = ((((uncomp_temp / 8) -
			  ((int32_t)dev->calib_param.dig_t1 << 1))) *
			((int32_t)dev->calib_param.dig_t2)) /
		       2048;
		var2 = (((((uncomp_temp / 16) -
			   ((int32_t)dev->calib_param.dig_t1)) *
			  ((uncomp_temp / 16) -
			   ((int32_t)dev->calib_param.dig_t1))) /
			 4096) *
				  ...
				  const struct bmp280_dev *dev)
{
	...
	rslt = null_ptr_check(dev);
	if (rslt == BMP280_OK) {
		var1 = (((int32_t)dev->calib_param.t_fine) / 2) -
		       (int32_t)64000;
		var2 = (((var1 / 4) * (var1 / 4)) / 2048) *
		       ((int32_t)dev->calib_param.dig_p6);
		var2 = var2 + ((var1 * ((int32_t)dev->calib_param.dig_p5)) * 2);
		var2 = (var2 / 4) +
		       (((int32_t)dev->calib_param.dig_p4) * 65536);
		var1 = (((dev->calib_param.dig_p3 *
			  (((var1 / 4) * (var1 / 4)) / 8192)) /
			 8) +
			((((int32_t)dev->calib_param.dig_p2) * var1) / 2)) /
		       262144;
		var1 = ((((32768 + var1)) *
			 ((int32_t)dev->calib_param.dig_p1)) /
			32768);
		*comp_pres = (uint32_t)(((int32_t)(1048576 - uncomp_pres) -
					 (var2 / 4096)) *
					3125);
		...
		if (var1 != 0) {
			...
			var2 = (((int32_t)(*comp_pres / 4)) *
				((int32_t)dev->calib_param.dig_p8)) /
			       8192;
			*comp_pres = (uint32_t)((int32_t)*comp_pres +
						((var1 + var2 +
...
uint8_t bmp280_compute_meas_time(const struct bmp280_dev *dev)
{
	...
	rslt = null_ptr_check(dev);
	if (rslt == BMP280_OK) {
		t_dur = period_per_osrs *
			((UINT32_C(1) << dev->conf.os_temp) >> 1);
		p_dur = period_per_osrs *
			((UINT32_C(1) << dev->conf.os_pres) >> 1);
	...
	if (rslt == BMP280_OK) {
		...
		if (rslt == BMP280_OK) {
			dev->calib_param.dig_t1 =
				(uint16_t)(((uint16_t)temp[BMP280_DIG_T1_MSB_POS]
					    << 8) |
					   ((uint16_t)
						    temp[BMP280_DIG_T1_LSB_POS]));
			dev->calib_param.dig_t2 =
				(int16_t)(((int16_t)temp[BMP280_DIG_T2_MSB_POS]
					   << 8) |
					  ((int16_t)
						   temp[BMP280_DIG_T2_LSB_POS]));
			dev->calib_param.dig_t3 =
				(int16_t)(((int16_t)temp[BMP280_DIG_T3_MSB_POS]
					   << 8) |
					  ((int16_t)
						   temp[BMP280_DIG_T3_LSB_POS]));
			dev->calib_param.dig_p1 =
				(uint16_t)(((uint16_t)temp[BMP280_DIG_P1_MSB_POS]
					    << 8) |
					   ((uint16_t)
						    temp[BMP280_DIG_P1_LSB_POS]));
			dev->calib_param.dig_p2 =
				(int16_t)(((int16_t)temp[BMP280_DIG_P2_MSB_POS]
					   << 8) |
					  ((int16_t)
						   temp[BMP280_DIG_P2_LSB_POS]));
			dev->calib_param.dig_p3 =
				(int16_t)(((int16_t)temp[BMP280_DIG_P3_MSB_POS]
					   << 8) |
					  ((int16_t)
						   temp[BMP280_DIG_P3_LSB_POS]));
			dev->calib_param.dig_p4 =
				(int16_t)(((int16_t)temp[BMP280_DIG_P4_MSB_POS]
					   << 8) |
					  ((int16_t)
						   temp[BMP280_DIG_P4_LSB_POS]));
			dev->calib_param.dig_p5 =
				(int16_t)(((int16_t)temp[BMP280_DIG_P5_MSB_POS]
					   << 8) |
					  ((int16_t)
						   temp[BMP280_DIG_P5_LSB_POS]));
			dev->calib_param.dig_p6 =
				(int16_t)(((int16_t)temp[BMP280_DIG_P6_MSB_POS]
					   << 8) |
					  ((int16_t)
						   temp[BMP280_DIG_P6_LSB_POS]));
			dev->calib_param.dig_p7 =
				(int16_t)(((int16_t)temp[BMP280_DIG_P7_MSB_POS]
					   << 8) |
					  ((int16_t)
						   temp[BMP280_DIG_P7_LSB_POS]));
			dev->calib_param.dig_p8 =
				(int16_t)(((int16_t)temp[BMP280_DIG_P8_MSB_POS]
					   << 8) |
					  ((int16_t)
						   temp[BMP280_DIG_P8_LSB_POS]));
			dev->calib_param.dig_p9 =
				(int16_t)(((int16_t)temp[BMP280_DIG_P9_MSB_POS]
					   << 8) |
					  ((int16_t)
			  ...
			  struct bmp280_dev *dev)
{
	...
	if ((rslt == BMP280_OK) && (conf != NULL)) {
		...
		if (rslt == BMP280_OK) {
			...
			rslt = bmp280_soft_reset(dev);
			if (rslt == BMP280_OK) {
				temp[0] = BMP280_SET_BITS(
					temp[0], BMP280_OS_TEMP, conf->os_temp);
				temp[0] = BMP280_SET_BITS(
					temp[0], BMP280_OS_PRES, conf->os_pres);
				temp[1] = BMP280_SET_BITS(temp[1],
							  BMP280_STANDBY_DURN,
							  conf->odr);
				temp[1] = BMP280_SET_BITS(
					temp[1], BMP280_FILTER, conf->filter);
				temp[1] = BMP280_SET_BITS_POS_0(
					temp[1], BMP280_SPI3_ENABLE,
					conf->spi3w_en);
		     ...
		     uint16_t length)
{
	...
	ret = i2c_master_send(aesd_bme_i2c_client_bmp, tx_data, length + 1);
		    ...
		    uint16_t length)
{
	...
	int ret = i2c_master_send(aesd_bme_i2c_client_bmp, &reg_addr, 1);
	ret |= i2c_master_recv(aesd_bme_i2c_client_bmp, reg_data, length);
...
*/
static int aesd_bmp280_probe(struct i2c_client *client,
			     const struct i2c_device_id *id)
{
	...
	pr_info("rslt : %d UP: %u, P32: %u \r\n", rslt, ucomp_data.uncomp_press,
		pres32);
...
*/
static void aesd_bmp280_remove(struct i2c_client *client)
{
	// put sensor in sleep mode
	bmp280_set_power_mode(BMP280_SLEEP_MODE, &bmp);
	pr_info("aesd bmp280 module removed\n");
}

/*
** Structure that has slave device id
*/
static const struct i2c_device_id aesd_bmp280_id[] = { { AESD_DEVICE_NAME, 0 },
						       {} };
MODULE_DEVICE_TABLE(i2c, aesd_bmp280_id);
...
static struct i2c_driver aesd_bmp280_driver = {
        ...
        .probe          = aesd_bmp280_probe,
        .remove         = aesd_bmp280_remove,
        .id_table       = aesd_bmp280_id,
};

/*
** I2C Board Info strucutre
*/
static struct i2c_board_info bmp_i2c_board_info = { I2C_BOARD_INFO(
...
static int __init aesd_bme_driver_init(void)
{
	...
	if ((alloc_chrdev_region(&aesd_bmp280_dev, 0, 1, "aesd_bme_dev")) < 0) {
		pr_err("Cannot allocate major number\n");
		return -1;
	}
	pr_info("Major = %d Minor = %d \n", MAJOR(aesd_bmp280_dev),
		MINOR(aesd_bmp280_dev));

	/*Creating cdev structure*/
	cdev_init(&aesd_bme_cdev, &fops);

	/*Adding character device to the system*/
	if ((cdev_add(&aesd_bme_cdev, aesd_bmp280_dev, 1)) < 0) {
	...
	/*Creating device*/
	if ((device_create(dev_class, NULL, aesd_bmp280_dev, NULL,
	...
	if (aesd_bme_i2c_adapter != NULL) {
		...
		aesd_bme_i2c_client_bmp = i2c_new_client_device(
			aesd_bme_i2c_adapter, &bmp_i2c_board_info);

		if (aesd_bme_i2c_client_bmp != NULL) {
	...
	pr_info("aesd bmp280 module initialized\n");
	return ret;
}
...
static void __exit aesd_bme_driver_exit(void)
{
	i2c_unregister_device(aesd_bme_i2c_client_bmp);
	i2c_del_driver(&aesd_bmp280_driver);
	device_destroy(dev_class, aesd_bmp280_dev);
	class_destroy(dev_class);
	cdev_del(&aesd_bme_cdev);
	unregister_chrdev_region(aesd_bmp280_dev, 1);
	pr_info("aesd bmp280 module removed");
}
